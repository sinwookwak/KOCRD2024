...
class MenubarManager:
    ...
    def callback_701(self):
        """열기 콜백."""
        logging.info(self.text_manager.get_text("log", "335")) # "File open action executed."
        self.system_manager.open_file_dialog()

    def callback_702(self):
        """저장 콜백."""
        logging.info(self.text_manager.get_text("log", "336")) # "File save action executed."
        self.system_manager.save_file()

    def callback_703(self):
        """종료 콜백."""
        logging.info(self.text_manager.get_text("log", "337")) # "Application exit action executed."
    ...
    def callback_704(self):
        ...
        if hasattr(self.system_manager, 'settings_manager'):
            ...
            QMessageBox.information(self.main_window,
                                    ...
                                    self.text_manager.get_text("log", "338")) # "Settings open action executed."
        else:
    ...
    def callback_705(self):
        """정보 콜백."""
        logging.info(self.text_manager.get_text("log", "339")) # "About dialog action executed."
    ...
    def open_deep_learning_dialog(self):
        ...
        model_file_filter = self.text_manager.get_text("general", "254") # "Model Parameters (*.traineddata);;All Files (*)"
        ...
        if file_path:
            logging.info(self.text_manager.get_text("log", "340").format(file_path=file_path)) # "AI model file selected: {file_path}"
            ...
            if hasattr(self.system_manager, 'ai_model_manager') and hasattr(self.system_manager.ai_model_manager, 'train_with_parameters'):
                 ...
                 logging.error(self.text_manager.get_text("error", "526")) # "AI Manager not properly configured."
                 QMessageBox.critical(self.main_window, self.text_manager.get_text("general", "501"), # "Error"
                                      self.text_manager.get_text("error", "526")) # "AI Manager not properly configured."
        else:
            logging.info(self.text_manager.get_text("log", "341")) # "No AI model file selected."
            QMessageBox.warning(
                self.main_window,
                dialog_title,
    ...
    def show_about_dialog(self):
        """정보 대화창을 표시합니다."""
        app_name = self.text_manager.get_text("general", "250") # "KOCRD"
        app_version = self.text_manager.get_text("general", "251") # "1.0"
        app_author = self.text_manager.get_text("general", "506") # "Unknown"
        about_text = f"{app_name}\n{app_version}\n{app_author}"

        QMessageBox.about(
            self.main_window,
            self.text_manager.get_text("general", "253"), # "About"
            about_text
        )
