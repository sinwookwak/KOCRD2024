...
class DocumentUI:
    ...
    def clear_table(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            QMessageBox.information(self.main_window, "초기화 완료", "파일 테이블이 초기화되었습니다.")
    ...
    def filter_documents(self, criteria):
        ...
        try:
            ...
            QMessageBox.warning(self.main_window, "필터 오류", "문서를 필터링 하는 중 오류가 발생했습니다.")

    def update_document_info(self, database_manager):
        """선택된 문서의 정보를 업데이트합니다."""
        selected_items = self.file_table.selectedItems()
        if not selected_items:
            QMessageBox.warning(self.main_window, "선택 오류", "수정할 문서를 선택하세요.")
            return
        ...
        if ok and new_type:
            try:
                ...
                QMessageBox.critical(self.main_window, "업데이트 오류", f"문서 유형을 업데이트 하는 중 오류가 발생했습니다: {e}")
    ...
    def delete_document(self, file_name, database_manager):
        ...
        try:
            if not file_name:
                ...
                if not selected_items:
                    QMessageBox.warning(self.main_window, "선택 오류", "삭제할 문서를 선택하세요.")
                    return
            ...
            QMessageBox.information(self.main_window, "삭제 완료", f"문서 '{file_name}'이(가) 삭제되었습니다.")
        except Exception as e:
            logging.error(f"Error deleting document: {e}")
            QMessageBox.critical(self.main_window, "삭제 오류", f"문서를 삭제하는 중 오류가 발생했습니다: {e}")

class DocumentUISystem:
    def __init__(self, main_window):
        self.main_window = main_window
        self.document_ui = DocumentUI(main_window)
    ...
    def init_ui(self):
        ...
        document_ui_widget = self.document_ui.create_table_widget()
